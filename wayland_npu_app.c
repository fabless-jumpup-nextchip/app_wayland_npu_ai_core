...
#include "nc_cnn_config_parser.h"       // ← 추가
...
void render(void *data, struct wl_callback *callback, uint32_t time)
{
    ...
    for(uint32_t ch = 0; ch < VIDEO_MAX_CH; ch++)
    {
        if (v4l2_config[ch].video_buf.video_fd == -1){
            ...
            if (det_buf) {
                ...
                // ↓ 추가: Adapter에 detection 결과 전달
                #ifdef SHOW_TRI_CHIMERA
                nc_trichimera_adapter_update(ch, det_buf,
                    nc_cnn_get_network_info(NETWORK_TRI_CHIMERA));
                #endif
            ...
            if (seg_buf) {
                ...
                // ↓ 추가
                #ifdef SHOW_TRI_CHIMERA
                nc_trichimera_adapter_update(ch, seg_buf, NULL);
                #endif
            }
            ...
            if (lane_buf) {
                ...
                // ↓ 추가
                #ifdef SHOW_TRI_CHIMERA
                nc_trichimera_adapter_update(ch, lane_buf, NULL);
                #endif
            }
    ...
    framecnt++;
    // wayland_npu_app.c — render 콜백 내부
    #ifdef SHOW_TRI_CHIMERA
        /* 30프레임마다 콘솔 출력 */
        if (framecnt % 30 == 0) {
            AIResult result;
            if (getTrichimeraResult(0, &result)) {
                printf("\n=== frame=%u ===\n", result.frame_id);

                /* Detection */
                int total_objs = 0;
                for (int c = 0; c < result.class_cnt; c++)
                    total_objs += result.det[c].obj_cnt;
                printf("objects=%d\n", total_objs);

                for (int c = 0; c < result.class_cnt; c++) {
                    for (int o = 0; o < result.det[c].obj_cnt; o++) {
                        AIObject *obj = &result.det[c].objs[o];
                        printf("  %s %.2f (x=%.1f y=%.1f w=%.1f h=%.1f)\n",
                            obj->class_name, obj->prob,
                ...
                /* Seg 요약 */
                printf("seg=%ux%u classes=%d map=%s\n",
                    result.seg.width, result.seg.height,
                    result.seg.class_cnt,
                    result.seg.seg_map ? "OK" : "NULL");

                /* seg_map은 getTrichimeraResult가 malloc했으므로 free 필요 */
                if (result.seg.seg_map) {
                    free(result.seg.seg_map);
                    result.seg.seg_map = NULL;
                }
            }
        }
    #endif
...
int main(int argc, char **argv)
{
    ...
    // 기존 nc_wayland_display_init() 등 초기화 이후
    nc_trichimera_adapter_init();
